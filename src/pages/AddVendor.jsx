  ...
  import {faSitemap,faBuildingNgo,faMapLocationDot,faLocationDot, faMobileRetro, faAt, faUserTie, faPenToSquare,faTrash, faMagnifyingGlass} from "@fortawesome/free-solid-svg-icons";
  ...
  const AddVendor = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      if (editingIndex === null) {
        ...
        if (checkDuplicateLocation(location)) {
          setDuplicateLocation("*Location already exist");
          loc = true;
        } else {
          setDuplicateLocation("");
        }
      }
      if (userEmail || org || loc) {
        return;
      }
      console.log("status",editingIndex !== null ? status: 1)
      setLoading(true);
      try {
        const response = await axios.post(
          `${Environment.BaseAPIURL}/OrgAddandUpdate`,
          {
            ...
            loggedInId: Uid,
            isActive: editingIndex !== null ? status: 1,
          }
        );
        if (response.data.message === "Successfully Inserted") {
          ...
          const insertData = await axios.post(`${Environment.BaseAPIURL}/InsertAuditReport`,{
              ...
              "ProcessName": "Organisation added successfully",
              "UserId": orgId,
              "CreatedBy": Uid,
              "TemplateId": "AddOrganisation"
            })
          ...
          const insertData = await axios.post(`${Environment.BaseAPIURL}/InsertAuditReport`,{
              ...
              "ProcessName": "Organisation updated successfully",
              "UserId": orgId,
              "CreatedBy": Uid,
              "TemplateId": "UpdateOrganisation"
            })
    ...
    const handleInputChange = (setter, fieldName) => (e) => {
      ...
      if (fieldName === "location") {
        if(e.target.value !== "" && checkDuplicateLocation(e.target.value)){
          setDuplicateLocation("*Location already exist");
        }
        else{
          setDuplicateLocation("")
    ...
    const handleDelete = async (deleteId) => {
      const selectedStateName = deleteId;
      const selectedState = stateCapture.find(
        (s) => s.state === selectedStateName
      );
      const stateId = selectedState ? selectedState.id : null;
      try {
        ...
        if(response.data.message === "Deleted Successfully"){
          const OrganisationId = response?.data?.orgId;
          const tableResponse = await axios.post(`${Environment.BaseAPIURL}/GetOrgReportData`, { action: "GetOrgReport" });
          const orgResponse = Array.isArray(tableResponse?.data) ? tableResponse?.data : [];
          setTableData(orgResponse);
          setFilterData(orgResponse);
          setTableDataForDuplicacy(orgResponse);
           const insertData = await axios.post(`${Environment.BaseAPIURL}/InsertAuditReport`,{
        ...
        <div className="main-vender-container">
          <form className="vendor-section-1" onSubmit={handleSubmit}>
            ...
            <div className="vendor-input">
              ...
              {duplicateLocation && <p className="error-msg">{duplicateLocation}</p>}
            </div>
          ...
          <div className="report-container-part-2" style={{ marginTop: "1rem" }}>
            <div className="report-search-button">
              ...
              <input type="text" placeholder="Search" className="report-search-input" onChange={handleSearch}/>
            </div>
              ...
              <tbody>
                {tableData.map((data, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <FontAwesomeIcon icon={faPenToSquare} onClick={() => handleEdit(index)} style={{
                          cursor: "pointer",
                          marginRight: "10px",
                          fontSize: "1.1rem",
                        }}/>
                      <FontAwesomeIcon icon={faTrash} style={{ cursor: "pointer", fontSize: "1.1rem" }}
                        onClick={() => {orgId.current = data.orgId;  setShowDeleteModal(true); setDeleteId(data.state);}}
                      />
                    </td>
                  </tr>
                ))}
